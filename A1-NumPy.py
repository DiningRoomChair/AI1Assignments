...
def fix_gauge_bias(car_data, speed_bias, rpm_bias):
    return car_data - [speed_bias, rpm_bias]
...
#  - The car from which this data originates has simple gears with consistent
...
def was_gear_switched(car_data):
    ...
    for x in range(len(car_data)):
        ratios.append(car_data[x, 1] / car_data[x, 0])
    for x in range(len(ratios) - 1):
        if ratios[x] - ratios[x + 1] > ratios[x] * 0.1:
            return True
    return False
...
def count_gears_used(car_data):
    ...
    if was_gear_switched(car_data) is False:
        return 1
    ratios = []
    unique_ratios = []
    for x in range(len(car_data)):
    ...
    for x in range(len(ratios)):
        is_unique = True
        for y in range(len(unique_ratios)):
            if y == x:
                continue
            if not(ratios[x] > unique_ratios[y] * 1.1 or ratios[x] < unique_ratios[y] * 0.9):
                is_unique = False
        if is_unique is True:
            unique_ratios.append(ratios[x])

    return len(unique_ratios)
...
if __name__ == '__main__':
    ...
    print("Score so for: " + str(total_score))
