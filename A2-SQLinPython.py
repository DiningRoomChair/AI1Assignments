...
def avgby(tbl, col, by):
    ...
    for x in range(len(tbl)):
        by_table.append(tbl[x][by])
    for x in range(len(by_table)):
        if by_table[x] is None:
            continue
        curr_index = x
        curr_value = by_table[curr_index]
        average = 0
        count = by_table.count(curr_value)
        for y in range(count):
            average += tbl[curr_index][col]
            by_table[curr_index] = None
            if curr_value not in by_table:
                continue
            curr_index = by_table.index(curr_value)
        average /= count
        return_dict[curr_value] = average
    return return_dict
...
def merge(left, right, left_on, right_on):
    ...
    for x in range(len(left)):
        for y in range(len(right)):
            if left[x][left_on] == right[y][right_on]:
                merged.append(left[x] + right[y])
    return merged
